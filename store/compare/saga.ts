...
import {actionTypes, getCompareListSuccess, updateCompareListSuccess} from './action';
...
function* addItemSaga(payload) {
  try {
    ...
    const localCompare = JSON.parse(JSON.parse(localStorage.getItem('persist:martfury')).compare);

    const existItem = localCompare.compareItems.find(item => item.id === product.id);
...
function* removeItemSaga(payload) {
  try {
    const {product} = payload;
    const localCompare = JSON.parse(JSON.parse(localStorage.getItem('persist:martfury')).compare);
    const index = localCompare.compareItems.indexOf(product);
    localCompare.compareTotal -= 1;
