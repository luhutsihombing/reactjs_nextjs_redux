...
import {consumerElectronic} from '../../../../public/static/data/home-1.json';
...
> {
  constructor(props) {
    super(props);
    this.state = {
      currentProducts: consumerElectronic.newArrivals as any,
          ...
          <div className="ps-section__header">
            ...
            <ul className="ps-section__links">
              {sectionLinks.map(link => (
                <li className={activeCategory === link.name ? 'active' : ''} key={link.name}>
                  {/* eslint-disable-next-line jsx-a11y/click-events-have-key-events */}
                  <a
                    role="button"
                    tabIndex={0}
                    onClick={e => this.handleChangeProduct(e, link.products, link.name)}>
                    {link.title}
                  </a>
                </li>
              ))}
