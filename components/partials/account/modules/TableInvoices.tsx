...
class TableInvoices extends PureComponent {
  render() {
    /*
    ...
    const tableColumn = [
      {
        ...
        key: 'invoiceId',
        width: '120px',
        render: (text, record) => <Link href="/account/invoice-detail">{record.invoiceId}</Link>
      },
      {
        title: 'Title',
        dataIndex: 'title',
      ...
      },
      {
        ...
        dataIndex: 'amount',
        key: 'amount',
        render: (text, record) => <span className="text-right">${record.amount}</span>
      },
      {
        title: 'Status',
        key: 'status',
    ...
    return <Table columns={tableColumn} dataSource={tableData} rowKey={record => record.id} />;
  }
}
